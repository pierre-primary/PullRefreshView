...
public class CanPullUtil {
    ...
    private static class AbsListViewCanPull implements Pullable {
        ...
        public boolean isGetTop() {
            int pt = absListView.getPaddingTop();
            if (absListView.getCount() == 0) {
                return true;
            } else if (absListView.getFirstVisiblePosition() == 0 && absListView.getChildAt(0).getTop() >= pt) {
                return true;
            }
            return false;
        }
        ...
        public boolean isGetBottom() {
            ...
            if (count == 0) {
                ...
                int pb = absListView.getPaddingBottom();
                View view = absListView.getChildAt(lastVisiblePosition - firstVisiblePosition);
                if (view != null && view.getBottom() <= absListView.getMeasuredHeight() - pb)
                    return true;
            }
            return false;
        }
